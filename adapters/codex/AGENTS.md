...
- `adapters/codex/plugins/impati-codex-tools/.codex-plugin/plugin.json`의 `version`을 Codex 플러그인 버전의 기준으로 사용한다.
- 플러그인 배포물에 변경이 생기면 SemVer 영향도를 판단하여 같은 변경에서 버전을 갱신한다.
- 저장소는 모든 어댑터에 같은 버전을 사용한다. Codex 배포물이 바뀌면 Claude 플러그인의 `version`도 같은 값으로 맞춘다.
