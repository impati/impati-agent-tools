#!/usr/bin/env python3
"""Synchronize shared core files into the installable Claude plugin bundle."""
...
def copy_skill(source: Path, destination: Path) -> None:
    """Copy a skill, dropping directories that only carry other agents' metadata."""
    shutil.copytree(
        source,
        destination,
        ignore=lambda directory, names: (
            set(names) & EXCLUDED_SKILL_DIRS if Path(directory) == source else set()
        ),
    )
...
def build_expected(destination: Path) -> None:
    ...
    for source_root in (CORE_SKILLS, ADAPTER_SKILLS):
        for skill_dir in sorted(path for path in source_root.iterdir() if path.is_dir()):
            ...
            copy_skill(skill_dir, skills_destination / skill_dir.name)


def synchronize(check_only: bool) -> None:
    with tempfile.TemporaryDirectory(prefix="impati-claude-plugin-") as temp_dir:
        ...
        if actual == expected:
            print("Claude 플러그인 배포물이 core 및 adapter 원본과 일치합니다.")
            return
        if check_only:
            ...
            raise RuntimeError(f"Claude 플러그인 배포물이 원본과 다릅니다. {details}")
        ...
        print("Claude 플러그인 배포물을 core 및 adapter 원본과 동기화했습니다.")
...
ADAPTER_SKILLS = REPO_ROOT / "adapters" / "claude" / "skills"
PLUGIN_ROOT = REPO_ROOT / "adapters" / "claude" / "plugins" / PLUGIN_NAME
